...
Name : faculty.h
Author : Jay Rathod
Description : This file provides API's to faculty to do tasks like adding course, modify course details, change password, view courses and logout.
============================================================================
*/
...
int remove_course(int connFD){
    ...
    if (readBytes == -1)
    {
        perror("Error while reading CourseID from client!");
        return 0;
    }
    ...
    if(ftPosition!=NULL) {
        ...
        write(connFD,"Invalid CourseID",15);
        return 0;
    }
    ...
    if (courseFileDescriptor == -1)
    {
        ...
        if (writeBytes == -1)
        {
            perror("Error while writing CourseID doesnt exists message to client!");
            return 0;
        }
        return 0;
    }
    ...
    if (errno == EINVAL)
    {
        ...
        bzero(writeBuffer, sizeof(writeBuffer));
        strcpy(writeBuffer,"CourseID doesn't exists");
        writeBytes = write(connFD, writeBuffer, strlen(writeBuffer));
        if (writeBytes == -1)
        {
...
int modify_course(int connFD){
    ...
    if(ftPosition!=NULL) {
        ...
        write(connFD,"Incalid CourseID ^",17);
        readBytes = read(connFD,readBuffer,sizeof(readBuffer));
        return 0;
    }
    ...
    if(courseID>course.id){
        write(connFD,"Invalid CourseID ^",20);
        readBytes = read(connFD,readBuffer,sizeof(readBuffer));
        return 0;
    }
    close(courseFileDescriptor);
...
int view_offering_course(int connFD){
    ...
    readBytes = read(connFD, readBuffer, sizeof(readBuffer));
    if (readBytes == -1){
        perror("Error getting CourseID from client!");
        return 0;
    }
    ...
    if(ftPosition!=NULL) {
        ...
        write(connFD,"Invalid CourseID ^",17);
        readBytes = read(connFD,readBuffer,sizeof(readBuffer));
        return 0;
    } 

...
Name : admin.c
Author : Jay Rathod 
Description : This file provides API's for admin to do tasks like adding student,faculty,viewing their details,modifying their details.
============================================================================
*/
...
int admin_operation_handler(int connFD)
{
    ...
    if(login_handler(1,connFD,NULL,NULL))
    {
        ...
        while (1)
        {
            ...
            case 9:
                ...
                break;            
            default:
                writeBytes = write(connFD,"Invalid Choice ^",14);
                readBytes = read(connFD,readBuffer,sizeof(readBuffer));
                break;
            }
        }
    ...
    {
        // ADMIN LOGIN FAILED
        write(1,ADMIN_LOGIN_FAILED,strlen(ADMIN_LOGIN_ID));
        return 0;
    }
    return 1;
}
...
int block_student(int connFD){
    ...
    if(ftPosition!=NULL) {
        ...
        write(connFD,"Invalid StudentID ^",17);
        readBytes = read(connFD,readBuffer,sizeof(readBuffer));
        return 0;
    }
    ...
    if(readBytes == -1){
        ...
        write(connFD,"Invalid StudentID ^",18);
        readBytes = read(connFD,readBuffer,sizeof(readBuffer));
        return 0;
    }
    lock.l_type = F_UNLCK;
    ...
    // Checking whether student is already blocked or not 
...
int get_student_details(int connFD)
{
    ...
    if (studentFileDescriptor == -1)
    {
        ...
        bzero(writeBuffer, sizeof(writeBuffer));
        strcpy(writeBuffer, "StudentID doesn't exists ^");
        writeBytes = write(connFD, writeBuffer, strlen(writeBuffer));
        if (writeBytes == -1)
        {
    ...
    if(studentID>fetchstudent.id){
        write(connFD,"Invalid StudentID ^",20);
        readBytes = read(connFD,readBuffer,sizeof(readBuffer));
        return 0;
    }
...
int get_faculty_details(int connFD)
{
    ...
    readBytes = read(connFD, readBuffer, sizeof(readBuffer));
    if (readBytes == -1){
        perror("Error getting FacultyID from client!");
        return false;
    }
    ...
    if (facultyFileDescriptor == -1)
    {
        ...
        bzero(writeBuffer, sizeof(writeBuffer));
        strcpy(writeBuffer, "FacultyID doesn't exists ^");
        writeBytes = write(connFD, writeBuffer, strlen(writeBuffer));
        if (writeBytes == -1)
        {
    ...
    if(facultyID>fetchfaculty.id){
        write(connFD,"Invalid FacultyID ^",20);
        readBytes = read(connFD,readBuffer,sizeof(readBuffer));
        return 0;
    }
...
int modify_student_info(int connFD){
    ...
    if (errno == EINVAL)
    {
        ...
        if (writeBytes == -1)
        {
            perror("Error while writing StudentUD doesnt exists message to client!");
            return false;
        }
        readBytes = read(connFD,readBuffer,sizeof(readBuffer));
        return false;
...
int modify_faculty_info(int connFD){
    ...
    if (errno == EINVAL)
    {
        ...
        if (writeBytes == -1)
        {
            perror("Error while writing FacultyID doesnt exists message to client!");
            return false;
        }
        return false;
    }
